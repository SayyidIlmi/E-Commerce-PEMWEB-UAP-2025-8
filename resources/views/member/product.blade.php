...
<head>
    ...
    <style>
        ...
        .detail-container { display: grid; grid-template-columns: 450px 1fr; gap: 40px; margin-bottom: 40px; }
        .main-image-card { background-color: #1f2937; border: 1px solid #374151; border-radius: 20px; padding: 20px; display: flex; align-items: center; justify-content: center; height: 400px; position: relative; overflow: hidden; }
        .main-image-card img { width: 100%; height: 100%; object-fit: contain; transition: 0.3s; }
        .main-image-card:hover img { transform: scale(1.05); }
        .thumbnail-list { display: flex; gap: 15px; margin-top: 15px; justify-content: center; }
        .thumb-item { width: 80px; height: 80px; background: #1f2937; border: 1px solid #374151; border-radius: 10px; padding: 5px; cursor: pointer; transition: 0.2s; }
        .thumb-item:hover, .thumb-item.active { border-color: #6366f1; transform: translateY(-3px); }
        .thumb-item img { width: 100%; height: 100%; object-fit: contain; }
        .product-title { font-size: 28px; font-weight: 700; color: white; margin-bottom: 10px; }
        .product-meta { display: flex; gap: 20px; color: #9ca3af; font-size: 14px; margin-bottom: 20px; align-items: center; }
        .rating-stars { color: #facc15; }
        .product-price { font-size: 32px; font-weight: 700; color: #6366f1; margin-bottom: 20px; background: rgba(99, 102, 241, 0.1); display: inline-block; padding: 5px 20px; border-radius: 10px; }
        .variant-group { margin-bottom: 25px; }
        .variant-label { display: block; color: #d1d5db; margin-bottom: 10px; font-weight: 500; }
        .variant-options { display: flex; gap: 10px; flex-wrap: wrap; }
        .variant-btn { background: #1f2937; border: 1px solid #374151; color: #d1d5db; padding: 8px 20px; border-radius: 8px; cursor: pointer; transition: 0.2s; font-size: 13px; }
        .variant-btn:hover, .variant-btn.selected { background: #6366f1; border-color: #6366f1; color: white; }
        .action-area { display: flex; gap: 20px; align-items: center; margin-top: 30px; border-top: 1px solid #374151; padding-top: 30px; }
        .qty-control { display: flex; align-items: center; background: #1f2937; border-radius: 50px; border: 1px solid #374151; }
        .qty-btn { background: none; border: none; color: white; padding: 10px 15px; cursor: pointer; font-size: 16px; }
        .qty-input { width: 50px; background: transparent; border: none; text-align: center; color: white; font-weight: 600; }
        .details-tabs { margin-top: 60px; }
        .tab-buttons { display: flex; gap: 30px; border-bottom: 1px solid #374151; margin-bottom: 30px; }
        .tab-link { padding-bottom: 15px; color: #9ca3af; cursor: pointer; font-size: 16px; font-weight: 500; position: relative; }
        .tab-link.active { color: #6366f1; }
        .tab-link.active::after { content: ''; position: absolute; bottom: 0; left: 0; width: 100%; height: 3px; background: #6366f1; }
        .tab-content { color: #d1d5db; line-height: 1.8; font-size: 14px; }
        .spec-table { width: 100%; border-collapse: collapse; }
        .spec-table td { padding: 10px 0; border-bottom: 1px solid #374151; }
        .spec-table td:first-child { width: 200px; color: #9ca3af; }
        @media (max-width: 992px) {
            .detail-container { grid-template-columns: 1fr; }
            .action-area { flex-direction: column; align-items: stretch; }
            .qty-control { justify-content: center; margin-bottom: 15px; }
        }
    </style>
</head>
<body class="dashboard-body">
            ...
            <div class="product-info-col">
                ...
                <form action="{{ route('dashboard', $product->id) }}" method="POST">
                    ...
                    <div class="action-area">
                        ...
                        <button type="submit" class="submit-btn" style="flex: 1; background: #6366f1;">
                ...
                <div style="margin-top: 15px; font-size: 12px; color: #9ca3af; display: flex; gap: 15px;">
                    <span><i class="fa-solid fa-shield-halved"></i> Garansi Resmi {{ $product->store->name ?? 'Toko' }}</span>
            ...
            @if(isset($related_products) && count($related_products) > 0)
                ...
                <div class="card product-card">
                    ...
                    <div class="product-info">
                        <h4><a href="{{ route('product.detail', $related->id) }}" style="text-decoration: none; color: inherit;">{{ $related->name }}</a></h4>
                        <span class="category-tag">{{ $related->category->name ?? 'Gadget' }}</span>
                        <div class="price-row">
                            <span class="price">Rp {{ number_format($related->price, 0, ',', '.') }}</span>
                            <a href="{{ route('product.detail', $related->id) }}" class="btn-icon"><i class="fa-solid fa-chevron-right"></i></a>
                        </div>
                    </div>
                </div>
                @endforeach
    ...
    <footer style="text-align: center; color: #6b7280; padding: 40px 0; border-top: 1px solid #374151; margin-top: 50px;">
